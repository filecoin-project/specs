...
After the chain has caught up to the current head using {{<sref chain_sync>}}. At a high-level, the mining process is as follows, (we go into more detail on epoch timing below):

- The node receives and transmits messages using the {{<sref message_syncer>}}
- At the same time it {{<sref block_sync "receives blocks">}}
...
- After an epoch starts (after the epoch "boundary"), the miner should take all the valid blocks received for the last epoch and assemble them into tipsets according to {{<sref tipset "tipset validation rules">}}
- The miner then attempts to mine atop the heaviest tipset (as calculated with {{<sref chain_selection "EC's weight function">}}) using its smallest ticket to run leader election
    ...
    - The miner uses these tickets in order to run {{<sref leader_election>}}
        ...
        - the miner then assembles a new block (see "block creation" below), sets an appropriate quantized timestamp for the epoch and broadcasts it
...
The lower row is the conceptual production channel made up of a period of mining `M` followed by a period of transmission `Tx` (which lasts long enough for blocks to propagate throughout the network).  The lengths of the periods are not to scale, a "faster" miner may release a block earlier and transmit longer than a "slower" miner.

The **epoch boundary** marks the beginning of a new epoch. It is at this wall-clock time that:

- the miner should start rejecting blocks from the past epoch.
- the miner can form tipsets and start mining atop them.
- the miner should timestamp new blocks made in this epoch with this epoch time.

Per the above diagram, in an epoch, blocks are received and validated during `Rx` up to the end of the epoch. At the boundary, the miner computes the heaviest tipset from the blocks received during `Rx`, and uses it as the head to build on during the next mining period `M`.  If mining is successful, the miner sets the block's timestamp to the prior epoch boundary and releases the block.
...
- Blocks received in the next epoch are not considered for inclusion in the current head tipset.
    - Note that those blocks are not invalid, just not considered for the miner's own tipset building. Tipsets received which include these blocks can be accepted.

In a fully synchronized network most of period `Rx` does not see any network traffic, only its beginning should. While there may be variance in operator mining time, all miners are expected to finish mining in the epoch.

Let's look at an example, both use a block-time of 30s, and a cutoff at 15s.

- T = 0: start of epoch n. Epoch cutoff for n-1, A assembles the heaviest tipset from n-1 and starts mining atop it.
- T = 12: A successfully generates a block, sets its timestamp to 0 and releases it.
- T in [0, 30]: miner A receives, validates and propagates incoming blocks. Valid blocks should have timestamp 0.
- T = 30: start of epoch n + 1. A forms tipsets for epoch n and starts mining atop the heaviest.
- T in [30, 60]: A receives and validates incoming blocks, their timestamp is 30.
- T = 60: start of epoch n + 2. A forms tipsets for epoch n + 1 and starts mining atop the heaviest. 

In practice miners should not be releasing blocks close to the epoch boundary (they will be done mining sooner). Implementations may choose to locally randomize the exact time of the cutoff for accepting new blocks in order to prevent such behavior. While this means such an implementation may include/not include blocks in their tipsets others do not, in practice this will not affect the miners submitting blocks on time.
