...
In short, the process of crafting a new ElectionProof in round N is as follows:
TODO: ADD LINK
...
The miner can then check whether they drew a winning election proof by comparing their power fraction (taking into account the number of blocks to be crafted per epoch `e`) to the `ElectionProof`s value, as follows:

`ElectionProof/maxValue < e * minerPower/totalPower`

using SHA-256 with our vdf, we would have `maxValue = 2^256`. The above is the integer format of `ElectionProof/2^256 < e * minerPower/totalPower` which makes it easy to see that a miner should win proportionally to the amount of power they have staked in the system. Note that miners with more than `1/e` fraction of power will win once at every round, meaning there may be fewer than e blocks per round with many large miners.
