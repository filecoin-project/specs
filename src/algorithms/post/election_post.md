...
3. **Verify `PartialTicket` returned**
    ...
    - Verify that no duplicate PartialTickets were submitted, that is tickets with the same `SectorID` and `challengeIndex` (tickets challenging the same sector at different indices are valid).
