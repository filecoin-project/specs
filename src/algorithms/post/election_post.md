...
At every epoch (on the order of 15s), each miner will challenge a portion (default to 100%) of sectors at random proportional to sectorsSampled, with each sector being issued K PoSt challenges (coverage may not be perfect).
...
1. (select eligible sectors) The miner calls GenerateCandidates from rust-proofs with their non-faulted (declared or detected) sectors along with a chosen number of ChallengeTickets (sectorsSampled*eligibleSectors).
...
1. (generate Challenge Ticket(s)) For each selected sector
    ...
    - Miner generates a ChallengeTicket using all the PoSt witnesses
    - ChallengeTicket = H(post_randomness || minerID ||S_ j || C_1_Output || … || C_K_Output)
1. (check Challenge Ticket(s) for winners) Given a returned ChallengeTicket, miner checks it is a winning ticket. Specifically, they do the following:
    - Set ElectionProof = H(ChallengeTicket) / 2^len(H)
    - Check that ElectionProof < activePowerInSector/networkPower * sectorsSampled * EC.ExpectedLeaders
...
1. (generate a PoStProof) for inclusion in the block header
    - Using the winning tickets from step 4 (there may be multiple tickets from the same SectorNumber), call GeneratePoSt from rust-proofs to generate a single PoSt for the block
...
At a high-level, to validate a winning PoSt Proof:
  1. Validate post_randomness:
    ...
    1. Rederive eligible sectors in order to verify that winning sectors were appropriately selected (meaning it’s in S_j) from {0, 1} ?= sectorID in (HashToSectorID(post_randomness || minerID || j))
    1. Verify ChallengeTickets returned
    - Prove that the ChallengeTickets were appropriately derived from the eligible sectors, by submitting all miner sectors along with the wanted number of tickets and verifying that the outputted ChallengeTickets match.
    1. Derive and validate the ElectionProof from the challengeTickets
    - Prove the derived ElectionProof is below the target
    {0, 1} ?= (ElectionProof < target)
    1. Validate the postProof using the ChallengeTickets
    - Verify that the postProof was appropriately derived from the ChallengeTickets.
    - The PoSt proof will verify the correctness of any ChallengeTickets passed to it as public inputs. In order to do this, it also needs the sector number along with various on-chain data (randomness, CommR, miner ID, etc.).
...
## Periodic PoSt cleanup

But while this means a miner will never win blocks from faked power, they won’t be penalized either when storage is lost. How do we ensure that the Power Table is accurate? Likewise, how do we onboard new power since it will not win a block unless it has at least 100TB or 10% of the network.

This is why we need PoSt cleanup: 

Whenever a miner is ProvingPeriod epochs (~2 days) past their last published PoSt, they will lose their power and a portion of their pledge collateral.

They will however have a CleanupPeriod starting before the end of the ProvingPeriod (~2 hours prior) in which the miner will be able to submit a PoSt proof to the blockchain in order to reset the ProvingPeriod.

This CleanupPoSt will use a challenge drawn from the chain at the start of this recovery period. Its PoStProof must be a proof over the ChallengeTickets for all sectors a miner is storing (i.e. a miner must submit a PoStProof made up of all challengeTickets for all eligible sectors, not just the winning ones on sampled sectors). For this reason a miner is incentivized to declare faults in order to successfully generate this PoStProof.

At the end of the ProvingPeriod/CleanupPeriod, the miner’s power will be cut and a portion of their pledge collateral slashed. 

Thereafter, the miner will have three more ProvingPeriods to recover their power by submitting a cleanupPoSt during one of the CleanupWindows. If the miner does not do so, their sectors are permanently terminated and their storage deal collateral slashed (see the StorageMinerActor in the spec).

Faults are largely independent from this process. The only rules are:
- Faults cannot be declared after the first cleanup period starts
- Faults cannot be recovered during a cleanup window
...
When such a PoSt pro is successfully submitted all faults are reset and assumed to be recovered. A miner must either (1) resolve a faulty sector and accept challenges against it in the next proof submission, (2) report a sector faulty again if it persists but is eventually recoverable, (3) report a sector faulty *and done* if the fault cannot be recovered.
...
{{% /notice %}}
                                       ...
                                       Cut power     Terminate sectors
                        Can submit     Slash some      Slash pledge
 Last PoSt             PoSt message      pledge         Slash deal
                         (chall)          (c)   (c)   (c)
     │                      │              │     │     │     │
  ───┼─────────────────────── ─ ─ ─ ─ ─ ─ ─│─ ─ ─ ─ ─ ─ ─ ─ ─│▶
     │                      │              │     │     │     │
     ◀──────────────────────▶◀────────────▶
          Election Period    Cleanup Period
                               (~2 hours)
     ◀─────────────────────────────────────▶
                   Proving Period

In the above diagram, here are possible miner actions:
Election Period
- Submit ElectionPoSt
CleanupWindow1
- Submit ElectionPoSt or CleanupPoSt
Any subsequent CleanupWindow
- Submit CleanupPoSt

Note that this first cleanup will frequently be used by small miners who do not win blocks, and by miners as they are onboarding power to the network, since they will not be able to win any blocks (they have no power) and hence gain power through ElectionPoSt.

While it resets a miner’s ElectionWindow, miners earn no reward from submitting CleanupPoSt messages. This mechanism does forces a miner to announce their faults and clean up the Power Table accordingly.
