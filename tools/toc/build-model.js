...
function buildTocModel(root) {
    ...
    function addSibling(node) {
        sectionNumber[sectionNumber.length - 1] =
            sectionNumber[sectionNumber.length - 1] + 1;
    ...
    for (let el of headingList) {
        ...
        if (!prevSibling || headingNum(node) === headingNum(prevSibling)) {
            ...
            let rmCount = 0;
            while (
                target <= headingNum(parents[parents.length - (rmCount + 1)])
            ) {
                rmCount++;
            }
            parents = parents.slice(0, parents.length - rmCount);
            sectionNumber = sectionNumber.slice(
                0,
                sectionNumber.length - rmCount
            );
...
function cleanHeadingText(el) {
    const anchorText = el.querySelector("a");
    if (anchorText) {
        return anchorText.textContent.trim();
    }
