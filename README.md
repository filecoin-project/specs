...
The specification is broken down into 4 levels (`#.#.#.#`). The L1 and L2 numbers in this sequence are determined by the first two directories extending from `/src/`; for example, `/src/systems/filecoin_mining/` resolves to `2.6.`. 

The L3 number is generated by creating an additional directory within a L2 directory, containing it's own appropriately formatted `index.md`. This new directory name must then be added to the `entries` field of the L2 `index.md` file, sequentially ordered as they are to be within the specification.

A further L4 subsection is added using the `##` header within a subsection's (in this case, the `sector` directory) `index.md` file.
