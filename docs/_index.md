...
The network maintains consensus over the current state of a replicated [state machine](./state-machine) through [expected consensus](./expected-consensus). This replicated state machine is used to run the [filecoin storage market](storage-market.md). This market provides a place to buy and sell storage within the distributed network of filecoin miners. The market also provides the needed mechanisms to ensure that the data stored by the network is actually being stored as promised, without requiring client interaction.
