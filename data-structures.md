...
A ticket is Fielcoin's version of a cryptographic lottery ticket and is used to prove that one is a block leader. At a high-level, tickets must do the following:
...
A signed message is a wrapper type over the base message. `SignedMessage` implements the `Signature` interface found in the [signatures specification](https://github.com/filecoin-project/specs/blob/master/signatures.md).

```go
type SignedMessage struct {

	// Addrs the Addresse associated with the Message in Msgs. This Address tracks 
  // the ID of the actor responsible for signing Msg. An ID can be converted into a 
  // public key via Address.GetPublicKey(ID).
	Addr Address

	// Msg is a Filecoin Message.
	Msg Message
