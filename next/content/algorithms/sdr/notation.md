...
$[a, b] = a, a + 1, \ldots, b$
The range of integers from $a$ up to and including $b$ (both endpoints are inclusive). There is an **ambiguity in notation** between this and the construction of a two element array. The notation $[a, b]$ **always refers to a range except in the cases where we pass a two element array as an argument into the hash functions $\Sha{254}_2$ and $\textsf{Poseidon}_2$**.
...
$\lower{2pt}{\llcorner} \int \lower{2pt}{\lrcorner_{2, \Le}}: \Bit^{[\lceil \log_2(\int) \rceil]}$
$\lower{2pt}{\llcorner} \int \lower{2pt}{\lrcorner_{8, \Le}}: [8]^{[\lceil \log_8(\int) \rceil]}$
...
$\DrgStringID: {\Byte_\utf}^{[*]} = ``\text{Filecoin_DRSample}"$
$\FeistelStringID: {\Byte_\utf}^{[*]} = ``\text{Filecoin_Feistel}"$
...
$\ParentLabels_{\mathbf{u}_\total}: \Label^{[d_\total]} =$
$\quad [\Label_{u_\drg, l} \mid \forall u_\drg \in \mathbf{u}_\drg] \concat [\Label_{u_\exp, l - 1} \mid \forall u_\exp \in \mathbf{u}_\exp]$
...
$\Bit_\Le^{[0 < n \leq \ell_\Fq^\bit]} \quad \as \Fq \quad\quad\thin\thin \\[4pt]
\Bit_\Le^{[0 < n \leq \ell_\Fqsafe^\bit]} \thin \as \Fqsafe \quad$
...
* The number of nodes always fits an unsigned 32-bit integer $N_\textsf{nodes} \leq 2^{32}$, and thus node indexes are representable using 32-bit unsigned integers.
