...
> **Definition:** A `TipSet` is a set of blocks with the same height, same parent set, and same number of tickets.
...
- At round N, the miner will draw the smallest ticket from the TipSet at round `N-K`, with `K` the randomness lookback parameter.
  - In the case where the tickets at round `N-K` are all losing tickets, i.e. if no block was generated in round `N-K` (see [Losing Tickets](#losing-tickets) for more), miners should attempt to generate their `ElectionProof` from that ticket which eventually leads to the creation of the block in the next TipSet with the smallest winning ticket, rather than the smallest block at round `N-K` (both are equivalent but this makes implementation simpler).
