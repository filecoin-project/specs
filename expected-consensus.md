...
2. Generate an ElectionProof = W
  i. T_o <-- DrawMinTicket(L rounds back)
  ii. W <-- VRF(SK, T_o)
3. Return: T, W
```
...
- At round N, the miner will sample the `ticket chain` at round `N-K`, with `K` the randomness lookback parameter.
  - This would typically mean the smallest ticket in the `TipSet` generated in that round, but in the case that round is did not yield any successful ElectionProofs (i.e. only generated losing tickets), miners should attempt to generate their new `ElectionProof` from the ticket generated by the eventual block generated with the smallest final ticket (i.e. not necessarily the smallest ticket generated at `N-K` -- this helps ensure liveness in case there are more than K successive losing tickets).
