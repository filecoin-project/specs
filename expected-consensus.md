...
2. Run it through VRF and get determinstic output
	i.   # take the VDFOutput of that ticket as input, concatenated with the operation type for ticket creation: "t"
				input <-- parentTicket.VDFOutput | "t"
...
3. Run it through VRF and get determinstic output
	i.   # take the VDFOutput of that ticket as input, concatenated with the operation type for election proof creation: "e"
				input <-- parentTicket.VDFOutput | "e"
	ii.	 # run it through the VRF and store the VRFProof in the new ticket
