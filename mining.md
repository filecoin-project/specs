...
```text
Input: received block, storage market actor S, miner's public key PK, a public parameter K, a public parameter L
Output: 0, 1
...
1. Determine the miner's power fraction
  ...
  ii. # Determine miner's power L rounds back
  		miner <-- block.miner
  		p_m <-- S.PowerLookup(miner)
  iii. # Get power fraction
  		p_f <-- p_m/p_n
2. Get the appropriate ticket from the ticket chain
...
3. Ensure that the scratched ticket is a winner
  	...
  	iii. # Compare the miner's scratchValue to the miner's power fraction
  			if scratchValue > p_f:
    			return 0
4. If winning ticket, verify Election Proof validity
