...
Once the miner has the aggregate `ParentState`, they must apply the block reward. This is done by adding the correct block reward amount to the miner owner's account balance in the state tree. The reward will be spendable immediately in the next block. See [block reward](#block-rewards) for details on how the block reward is structured. See [Notes on Block Reward Application](#notes-on-block-reward-application) for some of the nuances in applying block rewards.
...
Once a TipSet is produced and propagated, other nodes on the network will validate the TipSet. The full procedure for how to verify a TipSet can be found above in [Block Validation](#block-validation). At a high level, in order to validate a set of blocks in a TipSet, the validating node will first apply the block reward value directly to each of the parent nodes' accounts and then apply the messages to the resulting state. Thus, each of the miners who produced a block in the TipSet will receive a block reward. There will be no lockup. These rewards can be spent immediately (in the next block and later). 
