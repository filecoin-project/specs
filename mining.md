...
Thus, each of the miners who produced a block in the TipSet will receive a block reward. There will be no lockup imposed as a network-wide parameter, though it is assumed some implementations will choose to freeze withdrawals for some period based on their own security needs (and Filecoin's confirmation time). In cases this does not happen, these rewards can be spent immediately. 
